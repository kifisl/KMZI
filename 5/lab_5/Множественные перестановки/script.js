...
const text = TEXT.toLocaleLowerCase(),
    keyName = 'valeria',
    keySurname = 'troshko',
    keyNameArr = [0, 5, 1, 4, 6, 2, 3],
    keySurnameArr = [0, 4, 3, 6, 2, 5, 1],
    ROWS = keyName.length,
    COLUMS = keySurname.length;


let matrixText = [], entable = [];
...
function multiplyPermutationsEncrypt() {
    ...
    for (let i = 0; i < ROWS; i++) {
        entable[i] = [];
        for (let j = 0; j < COLUMS; j++) {
            entable[i][j] = matrixText[keyNameArr.indexOf(i)][keySurnameArr.indexOf(j)];
        }
    }
    ///////////////////////     FINISH     ///////////////////////
    ...
    document.querySelector('.encryptTime').innerHTML += timeFinish - timeStart + ' мс';
...
function multiplyPermutationsDecrypt(text) {
    ...
    document.querySelector('.decryptTime').innerHTML += timeFinish - timeStart + ' мс';
    return entable.flat().join('');
}
...
function Frequency(str, alph) {
    ...
    alph.forEach(letter => {
        letters[letter] = str.split(letter).length == 1 ? 0 : ((str.split(letter).length - 1) / str.length).toFixed(4);
    });
    console.log(letters);
    return letters;
}