...
namespace Cripta_Lab2
{
    class Entropia_Shennon
    {
        ...
        public double entrop(string str, string name_out_file = "Result.log")
        {
            ...
            foreach (char c in str)      //рассчет частоты появления символов
            {
            ...
            foreach(var el in collec)  //расчет энтропии по формуле Шеннона
            {
        ...
        public double info(string str, double entropy) //кол-во информации в сообщении
        {
            return str.Length * entropy;
        }

        public double infoMistake(int len, double p) //подсчет ошиб передачи единичного бита
        {
            double q = 1 - p;
